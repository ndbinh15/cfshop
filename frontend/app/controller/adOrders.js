angular.module('cfshop')
    .controller('adminOrderController', ['$scope', '$http', '$timeout', '$window', function ($scope, $http, $timeout, $window) {
        ...
        $scope.markDone = function (id) {
            console.log(id)
            // Make an HTTP POST request to mark the order as done
            $http.post("/orders/markAsDone?id=" + id)
                .then(function (response) {
                    console.log(response)
                })
                .catch(function (error) {
        ...
        $scope.getOrdersIn = function () {
            $http.get('/orders/getInprogress')
                .then(function (response) {
                    console.log(response)
                    $scope.readOrders = response.data;
                    $scope.totalPages = Math.ceil($scope.readOrders.length / $scope.itemsPerPage);
                    // Assign product numbers to each product
                    $scope.readOrders.forEach(function (product, index) {
        ...
        $scope.getOrdersCo = function () {
            $http.get('/orders/getCompleted')
                .then(function (response) {
                    console.log(response)
                    $scope.readOrders = response.data;
                    $scope.totalPages = Math.ceil($scope.readOrders.length / $scope.itemsPerPage);
        ...
        $scope.updateDisplayedProducts = function () {
            ...
            $scope.displayedProducts = $scope.readOrders.slice(startIndex, endIndex);
        };
        ...
        $scope.openShow2 = function () {
            $scope.getOrdersCo();
            $scope.isHiddenShow2 = !$scope.isHiddenShow2;
            $scope.isHiddenShow = true;
        }
